...
module.exports = {
  ...
  logout: async (req, res) => {
    let userId = res.locals.auth.parentId
      ? res.locals.auth.parentId
      : res.locals.auth.id;
  ...
  signin: (req, res) => {
    ...
    User.get(
      { phone: new RegExp(["^", req.body.phone, "$"].join(""), "i") },
      {
        name: 1,
        parentId: 1,
        username: 1,
      ...
      .then((result) => {
        ...
        if (result && result[0] && result[0].password === undefined) {
          res.status(500).json({
            success: false,
            message:
              "You haven't confirmed your account, please check your email",
            err,
          });
        }
       // eslint-disable-line no-console
        ...
        bcrypt
          ...
          .then((bcryptRes) => {
            if (bcryptRes) {
              User.get({ _id: result[0]._id, is_active: true }).then(
                async (active_user) => {
                  if (active_user.length > 0) {
                    ...
                    let rules = {};
                    if (result[0].role_id) {
                      let [role] = await Role.get({
                        _id: new mongoose.mongo.ObjectId(result[0].role_id),
                      });
                      if (role) {
                        rules = role.rules;
                      }
                    ...
                    if (result[0].parentId) {
                      let parentData = [];
                      parentData = await User.get({
                        _id: result[0].parentId,
                        is_active: true,
                      });
              ...
              res.status(400).json({
                success: false,
                message: "Phone or Password is not correct. Please try again",
              });
            }
          });
      })
  ...
  generateCodeForRegister: async (req, res) => {
    try {
      ...
      return res.json({
        success: false,
        data: {
          isNotValidNumber: !!dataMessages.find((message) =>
            message.includes("Not valid to_number")
          ),
          registerId: null,
        },
      });
